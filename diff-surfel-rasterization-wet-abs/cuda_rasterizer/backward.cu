...
renderCUDA(
	...
	// Modified: change the size of dL_dmeans2D from {P, 3} to {P, 4}, 2 for original gradient and 2 for the abs gradient
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (rho3d <= rho2d) {
				...
				// Added: Homodirectional gradients
				atomicAdd(&dL_dmean2D_abs[global_id].x, fabs(dL_dTu.z)); // not scaled
				atomicAdd(&dL_dmean2D_abs[global_id].y, fabs(dL_dTv.z)); // not scaled
			} else {
	 ...
	 glm::vec4* dL_drots)
{
	...
	if(dL_dmean2D.x != 0 || dL_dmean2D.y != 0)
	{
		...
		// Added: Homodirectional gradients
		dL_dmean2D_abs[idx].x += fabs(dL_dmean2D.z * f_vec.z * T[2].z);
		dL_dmean2D_abs[idx].y += fabs(dL_dmean2D.w * f_vec.z * T[2].z);
	...
	glm::vec4* dL_drots)
{
	...
	// Added: Homodirectional gradients
	dL_dmean2Ds[idx].z = dL_dmean2D_abs[idx].x * depth * 0.5 * float(W); // to ndc
	dL_dmean2Ds[idx].w = dL_dmean2D_abs[idx].y * depth * 0.5 * float(H); // to ndc
}
